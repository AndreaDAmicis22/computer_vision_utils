...
class DStadPrediction:
    heatmap: np.ndarray
    defects_mask: np.ndarray
    is_anomalous: bool


class DynamicStatefulAnomalyDetector:
    def __init__(
        self,
        ...
        self._inference_window_size = configuration.inference_window_size
    ...
    def _update_state(self, cls_token, cls_score, patch_tokens, patch_scores):
        self._memory_cls_features.append(cls_token)
        self._memory_patch_features.append(patch_tokens)
        self._good_cls_distances.append(cls_score)
        self._good_patch_scores.append(patch_scores)
    ...
    def _update_window_size(self):
        self._memory_cls_features = deque(maxlen=self._inference_window_size)
        self._memory_patch_features = deque(maxlen=self._inference_window_size)
        self._good_cls_distances = deque(maxlen=self._inference_window_size)
        self._good_patch_scores = deque(maxlen=self._inference_window_size)
    ...
    def predict(self, image: np.ndarray) -> DStadPrediction:
        cls_token, cls_score, patch_tokens, patch_scores = self._process_image(image)
        heatmap = self._create_heatmap(patch_scores)
        defects_mask = self._create_defects_mask(heatmap)
        is_anomalous = self._is_defected(cls_score)

        if not is_anomalous:
            self._update_state(cls_token, cls_score, patch_tokens, patch_scores)

        return DStadPrediction(heatmap, defects_mask, is_anomalous)
